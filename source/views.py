...
class IsolateViewSet(viewsets.ModelViewSet):
	...
	@action(detail=True, methods=['put'], name='Update Visibility')
	def update_visibility(self, request, pk=None, *args, **kwargs):
		try:
			user = request.user
			isolate = get_object_or_404(self.get_queryset(), pk=pk)
			serializer = self.get_serializer(isolate)
			...
			if (user.is_superuser or user.id == serializer.data['author_id']):
				if (Isolate.objects.filter(pk=pk).update(visibility=request.data['visibility'])):
					return Response(
						{'message': 'Isolate visibility successfully updated.'},
						status=status.HTTP_200_OK
					)
				else:
					return Response(
						{'error': 'Error while updating isolate visibility.'},
						status=status.HTTP_500_INTERNAL_SERVER_ERROR
					)
			else:
				return Response(
